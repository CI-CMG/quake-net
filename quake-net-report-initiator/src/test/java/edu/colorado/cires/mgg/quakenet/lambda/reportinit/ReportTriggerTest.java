...
class ReportTriggerTest {
  ...
  void testComplete() throws Exception {
    ...
    ReportInfoFile reportInfoFile = ReportInfoFile.Builder.builder().withStartTime(Instant.now().minusMillis(10000)).build();
    when(infoFileS3Actions.readReportInfoFile(eq(bucketName), eq("reports/2012/05/report-info-2012-05.json.gz"))).thenReturn(Optional.of(reportInfoFile));
    ...
    ReportTrigger reportTrigger = new ReportTrigger(properties, messageSender, bucketIteratorFactory, infoFileS3Actions, () -> now);
    ...
    verify(infoFileS3Actions, times(1)).saveReportInfoFile(
        eq(bucketName),
        eq("reports/2012/05/report-info-2012-05.json.gz"),
        eq(ReportInfoFile.Builder.builder(reportInfoFile).withStartReportGeneration(now).build())
        );

  }

  @Test
  void testCompleteNoInfo() throws Exception {
    String bucketName = "my-bucket";
    String topicArn = "topicArn";
    ReportInitiatorProperties properties = new ReportInitiatorProperties();
    properties.setBucketName(bucketName);
    ...
    when(infoFileS3Actions.readReportInfoFile(eq(bucketName), eq("reports/2012/05/report-info-2012-05.json.gz"))).thenReturn(Optional.empty());
    when(infoFileS3Actions.readInfoFile(eq(bucketName), any())).thenAnswer(new Answer<Optional<InfoFile>>() {
      @Override
      public Optional<InfoFile> answer(InvocationOnMock invocationOnMock) throws Throwable {
        String key = invocationOnMock.getArgument(1, String.class);
    ...
    verify(infoFileS3Actions, times(1)).saveReportInfoFile(
        eq(bucketName),
        eq("reports/2012/05/report-info-2012-05.json.gz"),
        eq(ReportInfoFile.Builder.builder().withStartTime(now).withStartReportGeneration(now).build())
    );
  ...
  void testNotComplete() throws Exception {
    ...
    verify(infoFileS3Actions, times(0)).saveReportInfoFile(eq(bucketName), any(), any());
